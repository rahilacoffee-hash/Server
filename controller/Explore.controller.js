...
export async function explore(req, res) { try { const limit = Math.min(50, Math.max(1, Number(req.query.limit) || 20)); const cursor = req.query.cursor ? new Date(req.query.cursor) : null; const match = cursor ? { createdAt: { $lt: cursor } } : {}; const posts = await Post.aggregate([{ $match: match }, { $addFields: { trendingScore: { $add: [{ $multiply: ["$likesCount", 4] }, { $multiply: ["$commentsCount", 5] }, { $multiply: ["$sharesCount", 8] }, { $multiply: ["$bookmarksCount", 6] }, "$viewsCount", { $multiply: [{ $divide: [{ $subtract: [new Date(), "$createdAt"] }, 3600000] }, -0.08] }] } } }, { $sort: { trendingScore: -1, createdAt: -1 } }, { $limit: limit + 1 }]); const hasMore = posts.length > limit; const result = posts.slice(0, limit); await Post.populate(result, { path: "user", select: "name username avatar" }); res.json({ success: true, data: { posts: result, nextCursor: hasMore ? result.at(-1).createdAt.toISOString() : null, hasMore } }); } catch (error) { res.status(500).json({ success: false, message: error.message }); } }
