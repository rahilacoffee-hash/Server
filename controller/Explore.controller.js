...
const postData = (post) => post.populate("user", "name username avatar isVerified");
...
export async function deletePost(req, res) {
  try {
    ...
    if (!post) return res.status(404).json({ success: false, message: "Post not found or you do not have permission to delete it" });
    await Promise.all([
      Like.deleteMany({ post: post._id }),
      Comment.deleteMany({ post: post._id }),
      PostView.deleteMany({ post: post._id }),
    ]);
    emit("postDeleted", { postId: post.id, userId: req.userId });
...
export async function explore(req, res) { try { const limit = Math.min(50, Math.max(1, Number(req.query.limit) || 20)); const cursor = req.query.cursor ? new Date(req.query.cursor) : null; const query = req.query.q?.trim(); const followingOnly = req.query.feed === "following"; const following = followingOnly ? (await User.findById(req.userId).select("following").lean())?.following || [] : []; const match = { ...(cursor ? { createdAt: { $lt: cursor } } : {}), ...(followingOnly ? { user: { $in: following } } : {}), ...(query ? { $or: [{ caption: { $regex: query, $options: "i" } }, { hashtags: { $regex: query.replace(/^#/, ""), $options: "i" } }] } : {}) }; const posts = await Post.aggregate([{ $match: match }, { $addFields: { trendingScore: { $add: [{ $multiply: ["$likesCount", 4] }, { $multiply: ["$commentsCount", 5] }, { $multiply: ["$sharesCount", 8] }, { $multiply: ["$bookmarksCount", 6] }, "$viewsCount", { $multiply: [{ $divide: [{ $subtract: [new Date(), "$createdAt"] }, 3600000] }, -0.08] }] } } }, { $sort: { trendingScore: -1, createdAt: -1 } }, { $limit: limit + 1 }]); const hasMore = posts.length > limit; const result = posts.slice(0, limit); await Post.populate(result, { path: "user", select: "name username avatar isVerified" }); const likedIds = new Set((await Like.find({ user: req.userId, post: { $in: result.map((post) => post._id) } }).select("post").lean()).map((like) => String(like.post))); res.json({ success: true, data: { posts: result.map((post) => ({ ...post, likedByMe: likedIds.has(String(post._id)), isOwner: String(post.user?._id) === String(req.userId) })), nextCursor: hasMore ? result.at(-1).createdAt.toISOString() : null, hasMore } }); } catch (error) { res.status(500).json({ success: false, message: error.message }); } }
